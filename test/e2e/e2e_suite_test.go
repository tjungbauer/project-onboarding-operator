...
var (
	...
	projectImage = defaultE2EImage()
)

func defaultE2EImage() string {
	data, err := os.ReadFile(filepath.Join("..", "..", "VERSION"))
	if err != nil {
		panic(fmt.Sprintf("read VERSION for e2e image tag: %v", err))
	}
	version := strings.TrimSpace(string(data))
	if version == "" {
		panic("VERSION file is empty")
	}
	return fmt.Sprintf("example.com/project-onboarding-operator:v%s", version)
}
...
func TestE2E(t *testing.T) {
	...
	_, _ = fmt.Fprintf(GinkgoWriter, "Starting project-onboarding-operator integration test suite\n")
	RunSpecs(t, "e2e suite")
}

var _ = BeforeSuite(func() {
	if openshiftE2EEnabled() {
		_, _ = fmt.Fprintf(GinkgoWriter, "OPENSHIFT_E2E=true: skipping Kind image build and CertManager setup\n")
		return
	}
